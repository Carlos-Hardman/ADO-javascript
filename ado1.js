...
function nomesDosAlunos() {
    return [ "Carlos Alberto Hardman Junior"];
}
    ...
    if(b > a && b > c && b > d){
        ...
        if(c > a && c > b && c > d){
            ...
            if(d > a && d > b && d > c){
               ...
               return " os numeros foram todos iguais";
            }
        }
    }
}
...
function abreviadorNomes(nomeCompleto) {
   const parte1donome = nomeCompleto.split(" ");
  if (parte1donome.length === 1) {
    return nomeCompleto;
  } else {
    const primeiroNome = parte1donome[0];
    const ultimoNome = parte1donome[parte1donome.length - 1];
